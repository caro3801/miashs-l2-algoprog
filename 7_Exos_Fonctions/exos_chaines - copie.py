def search(phrase: str, mot: str) -> int:
    """Renvoie l'indice du début de mot dans phrase, -1 si phrase ne contient pas mot"""
    longueur = len(mot)
...
def nb_mots(chaine: str) -> str:
    """Suppose que chaine contient des mots séparés par un seul espace.
        En ce cas, compter les mots consiste à compter les espaces et à ajouter 1...
        Remarque : il existe les fonction count() et split(), mais c'est de la triche"""
...
def bin_to_dec(chaine: str) -> int:
    ...
    accu = 0
    # Algo : on parcours la chaine en multipliant à chaque fois l'accu par 2 (méthode de Horner)
    for car in chaine:
        accu = (accu * 2) + int(car)
    return accu
...
def dec_to_bin(nb: int) -> str:
    ...
    # Algo : on procède par division successives en gardant les restes et on renvoie la chaine de restes inversée
    if nb == 0:
        return "0"
    res = ""
    while nb > 0:
...
if __name__ == '__main__':
    ...
    assert search("il était une fois", "il était une fois dans la ville de Foix") == -1
    assert search("", "était") == -1

    assert replace("il était une fois", "fois", "histoire") == "il était une histoire"
