...
def cree_combinaison_secrete():
    """Crée une combinaison de 6 couleurs prises parmi BRVJM
       Une couleur ne peut être choisie qu'une seule fois"""
    combi = ""
    deja_joue = ""          # Contient les couleurs déjà tirées au sort
    while len(combi) < 5:   # Il faut avoir choisi 5 couleurs...
        coul = COULEURS[aleatoire(0, 4)]
...
def traite_combinaison(tentative, secret):
    ...
    # sont modifiables...)
    # Une autre solution serait de passer par des 2 fois 5 variables, mais c'est trop artificiel
...
if __name__ == '__main__':
    ...
    while encore.upper() == "O":
        while bien_places != 4 and nb_essais < MAX_COUPS:
            essai = input("Essai " + str(nb_essais) + " (BRVJM) : ")
